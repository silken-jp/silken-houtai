...
const InspectionIssues: React.FC = () => {
  // state
  ...
  const genIssue = useRequest(genIssueType3, {
    manual: true,
    onSuccess: (data) => {
  ...
  return (
    <PageContainer
      header={{
        title: intlMenu('issues.inspectionIssues'),
        breadcrumb: {
          routes: [
            {
              path: '/issues/inspectionIssues',
              breadcrumbName: intlMenu('issues'),
            },
            {
              path: '',
              breadcrumbName: intlMenu('issues.inspectionIssues'),
            },
          ],
        },
      }}
