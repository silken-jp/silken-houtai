...
const Create: React.FC<CreateProps> = (props) => {
  ...
  useEffect(() => {
    if (visible) {
      form.setFieldsValue({
        JYO: 'Z',
        CHB: props?.dataSource?.[0]?.CHB,
        CH: '1A',
        ICD: props?.dataSource?.ICD,
        ST: props?.dataSource?.ST || '',
        TTC: props?.dataSource?.TTC || '',
  ...
  return (
    <>
      ...
      <Button type="primary" disabled={!props?.dataSource} onClick={handleOpen}>
        クリエート
      </Button>
    </>
  );
