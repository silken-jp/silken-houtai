...
const CargoIssues: React.FC = () => {
  ...
  const genIssue1 = useRequest(() => genIssueType4({ status: 'status1' }), {
    manual: true,
    onSuccess: (data) => {
  ...
  const genIssue2 = useRequest(() => genIssueType4({ status: 'status2' }), {
    manual: true,
    onSuccess: (data) => {
      message.success(data + '件更新しました。');
      refresh();
  ...
  return (
    <PageContainer
      header={{
        title: intlMenu('issues.cargoIssues'),
        breadcrumb: {
          routes: [
            {
              path: '/issues/cargoIssues',
              breadcrumbName: intlMenu('issues'),
            },
            {
              path: '',
              breadcrumbName: intlMenu('issues.cargoIssues'),
            },
          ],
        },
      }}
